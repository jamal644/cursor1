...
class MainPageLogic {
  ...
  Future getCurrentTransparency() async {
    final transparency =
        await SharedUtil.instance.getDouble(Keys.currentTransparency) ?? 1.0;
    ...
    return enableBg
        ? BoxDecoration(
            ...
            image: bgUrl.startsWith('http')
                ...
                : FileImage(File(bgUrl)) as ImageProvider<Object>,
            fit: BoxFit.cover,
          ))
        : BoxDecoration(
  ...
  Color getCurrentCardColor() {
    final context = _model.context;
    final primaryColor = Theme.of(context).primaryColor;
    final index = _model.currentCardIndex ?? 0;
    final taskLength = _model.tasks.length;
    ...
    switch (_model.currentAvatarType) {
      ...
      case CurrentAvatarType.net:
        ...
        FileUtil.getInstance().downloadFile(
          url: net,
          filePath: "/avatar/",
          fileName: (net != null && net.isNotEmpty
              ? net.split('/').last
              : "avatar.png"),
          onComplete: (path) {
            _model.currentAvatarUrl = path;
  ...
  Future getAvatarType() async {
    final currentAvatarType =
        await SharedUtil.instance.getInt(Keys.currentAvatarType) ?? 0;
