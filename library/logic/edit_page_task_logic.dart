      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            color: Colors.grey.withValues(
                alpha: ((Colors.grey.a * 255.0).round() & 0xff) * 0.2)),
        child: Row(
          children: <Widget>[
            icon,
            SizedBox(
...
//    });
  ...
  void pickEndTime(GlobalModel globalModel) {
    DateTime initialDate = _model.startDate;
    ...
    showDP(firstDate, initialDate, lastDate, globalModel.logic.isDarkNow())
        .then(
      (day) {
        if (day != null && day.isBefore(_model.startDate)) {
          showDialog(
  ...
  void pickStartTime(GlobalModel globalModel) {
    ...
    showDP(firstDate, initialDate, lastDate, globalModel.logic.isDarkNow())
        .then(
      (day) {
        if (day != null && day.isAfter(_model.deadLine)) {
          showDialog(
              context: _model.context,
              builder: (ctx) {
                return AlertDialog(
  ...
  Future<DateTime?> showDP(DateTime firstDate, DateTime initialDate,
      DateTime lastDate, bool isDarkNow) async {
    return showDatePicker(
      ...
      builder: (BuildContext context, Widget? child) {
        final color = _model.taskIcon.colorBean is ColorBean
            ? ColorBean.fromBean(_model.taskIcon.colorBean as ColorBean)
            : ColorBean.fromBean(ColorBean.fromColor(Colors.blue));
        return Theme(
          data: isDarkNow
              ? ThemeData.dark()
              : ThemeData(
                  ...
                  colorScheme: ColorScheme(
                    brightness: Brightness.light, // or .dark as appropriate
                    primary: Colors.blue,
  ...
  void postCreateTask(TaskBean taskBean, {bool isSubmitOldTask = false}) async {
    showDialog(
        context: _model.context,
        builder: (ctx) {
          return NetLoadingWidget(loadingController: _model.loadingController);
        });
  ...
  Future<TaskBean> transformDataToBean(
    ...
    TaskBean taskBean = TaskBean(
      taskName: taskName,
      account: account,
      taskStatus: _model.oldTaskBean.taskStatus,
      needUpdateToCloud: _model.oldTaskBean.needUpdateToCloud ?? '',
      ...
      textColor: _model.textColorBean is ColorBean
          ? _model.textColorBean as ColorBean
          : ColorBean.fromColor(Colors.black),
  ...
  void initialDataFromOld(TaskBean oldTaskBean) {
    ...
    _model.deadLine = oldTaskBean.deadLine.isNotEmpty
        ? DateTime.parse(oldTaskBean.deadLine)
        : DateTime.now();
    _model.startDate = oldTaskBean.startDate.isNotEmpty
        ? DateTime.parse(oldTaskBean.startDate)
        : DateTime.now();
    _model.createDate = oldTaskBean.createDate.isNotEmpty
        ? DateTime.parse(oldTaskBean.createDate)
        : DateTime.now();
    ...
    _model.taskIcon = oldTaskBean.taskIconBean is TaskIconBean
        ? oldTaskBean.taskIconBean as TaskIconBean
        : TaskIconBean(
            taskName: '',
            iconBean: IconBean(
                codePoint: 0,
