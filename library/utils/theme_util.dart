...
class ThemeUtil {
  ...
  Color getDarkColor(Color color) {
    int number = 20;
    int red = (((color.r * 255.0).round() & 0xff) - number <= 0)
        ? ((color.r * 255.0).round() & 0xff)
        : ((color.r * 255.0).round() & 0xff) - number;
    int green = (((color.g * 255.0).round() & 0xff) - number <= 0)
        ? ((color.g * 255.0).round() & 0xff)
        : ((color.g * 255.0).round() & 0xff) - number;
    int blue = (((color.b * 255.0).round() & 0xff) - number <= 0)
        ? ((color.b * 255.0).round() & 0xff)
        : ((color.b * 255.0).round() & 0xff) - number;
    return Color.fromRGBO(red, green, blue, 1);
  }

  Color getLightColor(Color color) {
    int number = 30;
    int red = (((color.r * 255.0).round() & 0xff) + number >= 255)
        ? ((color.r * 255.0).round() & 0xff)
        : ((color.r * 255.0).round() & 0xff) + number;
    int green = (((color.g * 255.0).round() & 0xff) + number >= 255)
        ? ((color.g * 255.0).round() & 0xff)
        : ((color.g * 255.0).round() & 0xff) + number;
    int blue = (((color.b * 255.0).round() & 0xff) + number >= 255)
        ? ((color.b * 255.0).round() & 0xff)
        : ((color.b * 255.0).round() & 0xff) + number;
    return Color.fromRGBO(red, green, blue, 1);
  }
  ...
  AppBarTheme getAppBarTheme(Color bgColor, Color iconColor) {
    return AppBarTheme(
      ...
      toolbarTextStyle: TextTheme(
        titleMedium: TextStyle(color: iconColor, fontSize: 20),
      ).bodyMedium,
